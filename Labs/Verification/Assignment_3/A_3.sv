module A_3;
import stim ::*;

bit clk, rst_n, tx_start, tx, tx_busy, parity_en, even_parity;
bit [7:0] data_in;
uart_tx dut (.*);
bit act_out [$];
bit exp_out [$]; 

transaction tr;

initial begin
    clk = 0;
    forever begin
        #5ns
        clk = ~clk;
    end
end

task automatic reset (ref bit rst_n);
    rst_n = 1;
    #1ns
    rst_n = 0;
    #1ns
    rst_n = 1;
endtask
function automatic void stim_size(int size,ref transaction tr);
    tr = new(size);
    exp_out = {};
    act_out = {};
endfunction

function automatic void gen_stim (ref transaction tr);
        assert (tr.randomize()) 
        else   $display("couldn't randomize stim\n");
endfunction

task automatic drive_stim (ref transaction tr, ref bit tx_start, tx_busy, parity_en, even_parity, clk, act_out [$],ref bit [7:0] data_in);
    foreach(tr.parity_en[i]) begin
        wait (tx_busy == 0)
        @(negedge clk)
        data_in = tr.data_in[i];
        parity_en = tr.parity_en[i];
        even_parity =tr.even_parity[i];
        tx_start    = 1'b1;
        @(negedge clk);
        tx_start = 1'b0;
        if(tr.parity_en[i]) begin    
            repeat (11)
                collect_out(clk, act_out);
        end else begin
            repeat (10)
                collect_out(clk ,act_out);
        end
    end
endtask

task automatic golden_model (ref transaction tr, ref bit tx_start, clk, exp_out [$]);
    bit parity;
    foreach(tr.parity_en[i]) begin
        exp_out.push_back(0);

        for(int j=0; j<8; j++) begin
            exp_out.push_back(tr.data_in[i][j]);
        end

        if(tr.parity_en[i]) begin
            parity = tr.even_parity[i] ? ~(^tr.data_in[i]) : ^tr.data_in[i];
            exp_out.push_back(parity);
        end

        exp_out.push_back(1);
    end
endtask

task automatic collect_out (ref bit clk, act_out [$]);  
       @(negedge clk)
       act_out.push_back(tx);
endtask

task automatic check_res (bit act_out [$], bit exp_out [$], ref transaction tr);
    int act_idx = 0, exp_idx = 0;
    bit [7:0] act_byte, exp_byte;
    bit act_parity, exp_parity;
    bit parity_check;

    foreach(tr.parity_en[i]) begin
        act_idx++;
        exp_idx++;

        for (int j=0; j<8; j++) begin
            act_byte[j] = act_out[act_idx];
            exp_byte[j] = exp_out[exp_idx];
            act_idx++;
            exp_idx++;
        end

        if (tr.parity_en[i]) begin
            act_parity = act_out[act_idx];
            exp_parity = exp_out[exp_idx];
            act_idx++;
            exp_idx++;
            parity_check = act_parity == exp_parity;
        end
        act_idx++;
        exp_idx++;
        if (act_byte == exp_byte && parity_check) begin
            $display("Testcase No. %0d: [PASS] Transmitted Byte : %b, Parity= %b Expected Byte : %b Expected parity: %b", i, act_byte, act_parity, exp_byte, exp_parity);
        end else begin
            $display("Testcase No. %0d: [ERROR] Transmitted Byte : %b, Parity= %b Expected Byte : %b Expected parity: %b", i, act_byte, act_parity, exp_byte, exp_parity);
        end
    end
endtask

initial begin
    reset (rst_n);
    stim_size(500, tr);
    gen_stim (tr);
    drive_stim (tr,tx_start, tx_busy, parity_en, even_parity, clk, act_out, data_in);
    golden_model (tr, tx_start, clk, exp_out);
    check_res (act_out, exp_out, tr);
end
endmodule 




